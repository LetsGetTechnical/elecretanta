 ...
 * @param {number} [props.initialStep] - The initial step to display in the onboarding flow.
...
export const Onboarding = ({
  ...
  useEffect(() => {
    /**
     * Fetches the user's avatar asynchronously and updates the avatar state.
  ...
  useEffect(() => {
    /**
     * Fetches the user profile and sets form values accordingly.
     * @returns {Promise<void>}
     */
  ...
  /**
   * Advances the onboarding form to the next step if not at the last step.
   * @returns {void}
   */
  function next(): void {
  ...
  /**
   * Moves the onboarding form to the previous step if not at the first step.
   * @returns {void}
   */
  function back(): void {
    if (currentStep > 0) {
  ...
  /**
   * Handles form submission for the onboarding steps.
   * Submits the profile update on the last step, otherwise advances to the next step.
   * @returns {Promise<void>}
   */
