      ...
      {isLoading ? (
        <>
          <div className="flex justify-center items-center h-full">
            <div data-testid="loading-spinner" className="border-t-4 border-red-500 border-solid w-12 h-12 rounded-full animate-spin"></div>
          </div>
        </>
      ) : (
        <>
          ...
          <div className="flex flex-col justify-center mt-4 gap-4">
            {[
              ...
              <button
                key={index}
                data-testid={`feedback-button-${index}`}
